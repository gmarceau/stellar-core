...
TEST_CASE("core topology at multiple scales", "[simulation]")
{
    int const nLedgers = 3;
    std::vector<std::chrono::seconds> times;

    for (int nNodes = 2; nNodes < 30; nNodes++)
    {
        auto tBegin = std::chrono::system_clock::now();

        Simulation::pointer simulation = Topologies::core(nNodes, 0.5, Simulation::OVER_LOOPBACK);
        simulation->startAllNodes();

        simulation->crankUntil(
            [&simulation, nLedgers]()
        {
            return simulation->haveAllExternalized(nLedgers);
        },
            nLedgers * Herder::EXP_LEDGER_TIMESPAN_SECONDS);
        ...
        LOG(INFO) << "Time spent closing " << nLedgers << " ledgers with " << nNodes << " nodes : "
            << t.count() << " seconds";

        LOG(INFO) << simulation->metricsSummary("scp");
    }
}

TEST_CASE("hierarchical topology at multiple scales", "[simulation]")
{
    int const nLedgers = 3;
    std::vector<std::chrono::seconds> times;
